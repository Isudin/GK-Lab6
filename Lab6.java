...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public int numOfStages = 14;
//	public float x, y, z;
	public float x=0, y, z=0;
	...
	public void Pyramid(GL2 gl2){
		double angle = 0;
			for(int i = 0; i <= numOfStages; i++)
			{
				...
				gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[7], 1);
				gl2.glVertex3f(x, y-1, z);		
				
				gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[3], 1);
				gl2.glVertex3f(0, 5, 0);
				
				x = (float) (Math.sin(i * (2 * Math.PI / numOfStages)) * radius);
				z = (float) (Math.cos(i * (2 * Math.PI / numOfStages)) * radius);
				gl2.glVertex3f(x, y-1, z);
...
//		float[] light = { 1f,1f,1f,1.2f };
//        gl2.glLightModeli(GL2.GL_LIGHT_MODEL_TWO_SIDE,2);
//        gl2.glLightModelfv( GL2.GL_LIGHT_MODEL_AMBIENT, light,1 );
//		gl2.glLightfv(GL2.GL_LIGHT0, GL2.GL_SPOT_DIRECTION, light, 0); 
	}
